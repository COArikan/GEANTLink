...
wxGTCPConfigPanel::wxGTCPConfigPanel(const eap::config_provider &prov, eap::config_method_eapgtcp &cfg, wxWindow* parent) : wxPanel(parent)
{
    wxBoxSizer* sb_content;
    sb_content = new wxBoxSizer( wxVERTICAL );

    m_credentials = new wxGTCPCredentialsConfigPanel(prov, cfg, this, _("User ID and GTC Password"));
    sb_content->Add(m_credentials, 0, wxEXPAND, 5);
    ...
    // Connect Events
    this->Connect(wxEVT_INIT_DIALOG, wxInitDialogEventHandler(wxGTCPConfigPanel::OnInitDialog));
}


wxGTCPConfigPanel::~wxGTCPConfigPanel()
{
    // Disconnect Events
    this->Disconnect(wxEVT_INIT_DIALOG, wxInitDialogEventHandler(wxGTCPConfigPanel::OnInitDialog));
}


/// \cond internal
void wxGTCPConfigPanel::OnInitDialog(wxInitDialogEvent& event)
{
    // Forward the event to child panels.
    if (m_credentials)
        m_credentials->GetEventHandler()->ProcessEvent(event);
